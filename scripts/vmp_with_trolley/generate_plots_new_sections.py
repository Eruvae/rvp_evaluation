...
def generatePlots(time_names, value_names, times_rvp, values_rvp, segments_rvp, times_vmp, values_vmp, segments_vmp, out_folder):
    for i in range(len(time_names)):
        for j in range(len(value_names)):
            ...
            print(figname, values_rvp[j][-1], values_vmp[j][-1])
...
rvp_folder = 'rvp/t12_w22_t60/'
vmp_folder = 'vmp/t14_w22_t60/'
...
times_rvp, values_rvp, time_names_rvp, value_names_rvp, segment_starts_rvp = readValues(file_rvp, time_columns, columns_ec, seg_column=22)
segments_rvp = values_rvp[-1]
times_vmp, values_vmp, time_names_vmp, value_names_vmp, segment_starts_vmp = readValues(file_vmp, time_columns, columns_ec, seg_column=22)
segments_vmp = values_vmp[-1]

times_rvp, times_vmp = moveSegmentStartTimes(times_rvp, times_vmp, segment_starts_rvp, segment_starts_vmp)

times_rvp_old, values_rvp_old, time_names_rvp_old, value_names_rvp_old, _ = readValues(file_rvp_old, time_columns, columns_old, seg_column=20)
segments_rvp_old = values_rvp_old[-1]
times_vmp_old, values_vmp_old, time_names_vmp_old, value_names_vmp_old, _ = readValues(file_vmp_old, time_columns, columns_old, seg_column=20)
